...
//其前54个字节未文件头信息，从第55个字节开始到结束的所有数据
#include <stdio.h>
#include <stdlib.h>
#include <winsock.h>
#include <windows.h>
...
int main(int argc, char* argv[])
{
	if (argc<3)
	{
		printf("Usage : %s Encrypt secret_file_name BMP_file_name \n", argv[0]);
		printf("Usage : %s Decrypt BMP_file_name secret_file_name \n", argv[0]);
		return 0;
	}
...
// HANDLE hbmp = CreateFileA(".\\screenshot.bmp", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE | FILE_SHARE_READ, NULL, OPEN_EXISTING, NULL, NULL);
// if (hbmp == INVALID_HANDLE_VALUE)
// {
// 	printf("Can't open srceenshot.bmp\n");
// 	return -1;
// }
// DWORD dwRead;
// DWORD dwSize = GetFileSize(hbmp, &dwRead);
// char * lpBMP = new char[dwSize];
// RtlZeroMemory(lpBMP,sizeof(lpBMP));
// ReadFile(hbmp, lpBMP, dwSize, &dwRead, 0);
// if (dwRead != dwSize)
// {
// 	printf("Read screenshot.bmp failed\n");
// 	return -1;
// }

// HANDLE hsecret = CreateFileA(".\\secret.txt", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE | FILE_SHARE_READ, NULL, OPEN_EXISTING, NULL, NULL);
// if (hsecret == INVALID_HANDLE_VALUE)
// {
// 	printf("Can't open secret file\n");
// 	return -1;
// }
// DWORD dwSecretSize = GetFileSize(hsecret, &dwRead);
// char * lpSecret = new char[dwSecretSize];
// RtlZeroMemory(lpSecret,sizeof(lpSecret));
// ReadFile(hsecret, lpSecret, dwSecretSize, &dwRead, 0);
// if (dwRead != dwSecretSize)
// {
// 	printf("Read secret file failed\n");
// 	return -1;
// }